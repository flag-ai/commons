...
class InvalidTokenError(InstallScriptError):
    """The registration token is not a safe alphanumeric string."""


def _is_local_host(host: str) -> bool:
    """Loopback or private-range IP literal, or a plain localhost name."""
...
def validate_server_url(url: str, *, allow_insecure: bool = False) -> str:
    """Accept only ``https://host[:port][/path]`` and return it normalised.
    ...
    The rendered script POSTs the new agent's auth token to this URL, so
    ``http`` is accepted only for loopback/private-IP hosts or with
    ``allow_insecure=True``.
    """
    raw = (url or "").strip()
    ...
    if not SAFE_HOST.fullmatch(host):
        raise InstallScriptError("server_url contains unsafe characters")
    if parts.scheme == "http" and not allow_insecure and not _is_local_host(host):
        raise InstallScriptError(
            "server_url must use https (the agent posts its auth token to it)"
        )
    try:
        port = parts.port
    ...
    if port == 0:
        raise InstallScriptError("server_url has an invalid port")
...
def render_install_script(
    ...
    *,
    ...
    values = {
        ...
        "{{.ServerURL}}": shlex.quote(
            validate_server_url(server_url, allow_insecure=allow_insecure)
        ),
...
def install_command(
    ...
    *,
    ...
    base = validate_server_url(server_url, allow_insecure=allow_insecure)
    url = f"{base}{script_path}?token={validate_token(token)}"
